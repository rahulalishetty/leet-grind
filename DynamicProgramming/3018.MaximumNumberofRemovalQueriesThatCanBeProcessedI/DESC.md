...
**Explanation:**
We don't do any operation and process the queries as follows:
...
5. We cannot choose any elements from `nums` since they are not greater than or equal to `5`.

Hence, the answer is `4`. It can be shown that we can't process more than `4` queries.
...
Hence, the answer is `3`. It can be shown that we can't process more than `3` queries.
...
**Explanation:**
First, we replace `nums` with the subsequence of `nums` `[4,3]`.
...
3. We cannot process any more queries since `nums` is empty.

Hence, the answer is `2`. It can be shown that we can't process more than `2` queries.
