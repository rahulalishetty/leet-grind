...
We can see that the subtree rooted at node `0` has children with different colors. Any other subtree is of the same color and has a size of `1`. Hence, we return `1`.
...
**Explanation:**
Each color is represented as: `1 -> Red`, `2 -> Green`, `3 -> Blue`.
We can see that the subtree rooted at node `0` has children with different colors. Any other subtree is of the same color, but the subtree rooted at node `2` has a size of `3` which is the maximum. Hence, we return `3`.
