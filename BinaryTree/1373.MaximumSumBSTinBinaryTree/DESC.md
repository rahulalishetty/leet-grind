# 1373. Maximum Sum BST in Binary Tree

## Problem Statement

Given a binary tree `root`, return the **maximum sum of all keys of any subtree** which is also a **Binary Search Tree (BST)**.

Assume a BST is defined as follows:

- The **left subtree** of a node contains only nodes with keys **less than** the node's key.
- The **right subtree** of a node contains only nodes with keys **greater than** the node's key.
- Both the **left and right subtrees must also be binary search trees**.

---

## Example 1

![alt text](image.png)

**Input**

```
root = [1,4,3,2,4,2,5,null,null,null,null,null,null,4,6]
```

**Output**

```
20
```

**Explanation**

The maximum sum in a valid BST subtree is obtained from the subtree rooted at node `3`.

---

## Example 2

![alt text](image-1.png)

**Input**

```
root = [4,3,null,1,2]
```

**Output**

```
2
```

**Explanation**

The maximum sum BST is just the single node `2`.

---

## Example 3

**Input**

```
root = [-4,-2,-5]
```

**Output**

```
0
```

**Explanation**

All values are negative. The best choice is to return the sum of an empty BST, which is `0`.

---

## Constraints

- The number of nodes in the tree is in the range **[1, 4 × 10⁴]**
- **-4 × 10⁴ ≤ Node.val ≤ 4 × 10⁴**
