...
- The characters added do **not necessarily need to be the same** as those removed.
- However, **both operations must be performed every second**.

Your task is to return the **minimum time greater than zero** required for `word` to **revert back to its initial state**.
